    ...
    const fetchAnalytics = async () => {
      try {
        ...
        if (res.data && res.data.success) {
          ...
          // 1. Continuous 6-Month Timeline (Apr - Sep) so line chart connects smoothly across months
          ...
          for (let i = 5; i >= 0; i--) {
            ...
            const val = retailerGrowthTrend && retailerGrowthTrend[upperMonth] ? Number(retailerGrowthTrend[upperMonth]) : 0;
          ...
          if (retailerDistrictDistribution && Object.keys(retailerDistrictDistribution).length > 0) {
            entries = Object.entries(retailerDistrictDistribution).map(([k, v]) => [k, Number(v)]);
          }

          if (entries.length === 0) {
            entries = [['Colombo', 2], ['Gampaha', 1], ['Kandy', 1]];
          }
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="text-sm font-extrabold text-gray-900 uppercase tracking-wider">
                    Retailer Regional Distribution
                  </h3>
                ...
                <p className="text-[11px] text-gray-400 font-semibold mt-1">
                  ...
                  {graphView === 'bar' && 'Registered retailer & consumer counts ranked by district'}
                  ...
                  <div className="md:col-span-6 relative flex items-center justify-center h-[260px]">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        ...
                        <Tooltip
                          ...
                          itemStyle={{ color: '#fff' }}
                          formatter={(value: any, name: any) => [`${value} Retailers`, `${name}`]}
                        />
                      </PieChart>
                    </ResponsiveContainer>
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none select-none">
                      <span className="text-3xl font-black text-gray-900 tracking-tight">
                        {hoveredSlice ? hoveredSlice.value : (analytics.totalRetailers || districtList.reduce((s, d) => s + d.value, 0))}
                      </span>
                      <span className="text-[10px] font-extrabold uppercase tracking-wider text-[#1e4d1e] mt-0.5 text-center max-w-[110px] truncate">
                        {hoveredSlice ? hoveredSlice.name : 'Total Retailers'}
                      </span>
                      <span className="text-[9px] font-semibold text-gray-400">
                        {hoveredSlice
                          ? `${Math.round((hoveredSlice.value / (analytics.totalRetailers || 1)) * 100)}% of total`
                          : 'Registered'}
                      </span>
                    </div>
                  </div>
                      ...
                      {districtList.map((district) => (
                        ...
                        >
                          <div className="flex items-center justify-between text-xs font-bold mb-1">
                            ...
                            <div className="flex items-center gap-2">
                              <span className="text-gray-500 text-[11px]">{district.value} {district.value === 1 ? 'retailer' : 'retailers'}</span>
                              <span
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={districtList} margin={{ top: 20, right: 24, left: -10, bottom: 10 }}>
                      ...
                      <Tooltip
                        ...
                        itemStyle={{ color: '#fff' }}
                        formatter={(val: any) => [`${val} Retailers`, 'Registered']}
                      />
                      <Bar
                        dataKey="value"
                        radius={[8, 8, 0, 0]}
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={chartDataList} margin={{ top: 20, right: 24, left: -12, bottom: 0 }}>
                      ...
                      <Tooltip
                        ...
                        formatter={(value: any) => [`${value} New Retailers`, 'Monthly Growth']}
                      />
                      <Area
                        type="monotone"
                        dataKey="value"
